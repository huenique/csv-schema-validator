...
class GitHubActionsReporter:
    ...
    def report_results(self, results: Dict[str, ValidationResult]) -> None:
        ...
        if self.is_github_actions:
            print(f"::notice::CSV Validation Summary: {passed_files}/{total_files} files passed")
        ...
        for file_path, result in results.items():
            if result.is_valid:
                if self.is_github_actions:
                    print(f"::notice file={file_path}::✅ Validation passed ({result.total_rows} rows)")
                print(f"✅ {file_path}: PASSED ({result.total_rows} rows)")
            else:
                if self.is_github_actions:
                    print(f"::error file={file_path}::❌ Validation failed - {result.error_count} errors, {result.warning_count} warnings")
                ...
                for issue in result.issues[:3]:
                    if self.is_github_actions and issue.severity == "error":
                        row_info = f" at line {issue.row_number}" if issue.row_number else ""
                        print(f"::error file={file_path}{row_info}::{issue.field_name}: {issue.description}")
...
def generate_ci_report(results: Dict[str, ValidationResult], 
...
def create_github_workflow(output_path: Path = None) -> str:
    """
  ...
  validate-csv:
    ...
    - name: Generate validation report
      ...
      run: |
        ...
        from csv_schema_validator.test_runner import SchemaTestRunner
        import sys
...
from pathlib import Path
from csv_schema_validator.integrations.ci_cd import GitHubActionsReporter, generate_ci_report
from csv_schema_validator.test_runner import SchemaTestRunner
