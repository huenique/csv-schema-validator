...
from .validators import BatchValidator, CSVSchemaValidator, ValidationResult
...
class SchemaTestRunner:
    ...
    def discover_csv_files(self, patterns: Optional[Dict[str, str]] = None) -> Dict[str, List[Path]]:
        """
        ...
        for schema_type, pattern in patterns.items():
            ...
            files = [f for f in files if "test_results" not in str(f) and "__pycache__" not in str(f)]
    ...
    def run_full_validation(self, include_warnings: bool = True) -> Dict[str, Any]:
        ...
        print(f"Found {total_files} CSV files across {len(discovered_files)} schema types")
        ...
        test_summary = self._generate_test_summary(all_results, execution_time, include_warnings)
    ...
    def run_directory_validation(self, directory: Union[str, Path]) -> Dict[str, ValidationResult]:
        """
    ...
    def run_regression_test(
        self,
        baseline_results_file: Optional[Union[str, Path]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def generate_detailed_report(
        ...
        if format_type == "html":
            ...
            report_content = json.dumps(self._make_serializable(results), indent=2, default=str)
        else:
    ...
    def _generate_test_summary(
        ...
        return {
            "summary": {
                ...
                "success_rate": (len(passed_files) / total_files * 100) if total_files > 0 else 0,
            },
            "issue_counts": {
    ...
    def _print_final_summary(self, test_summary: Dict[str, Any]) -> None:
        ...
        if test_summary["failed_files"]:
            ...
            for file_path in test_summary["failed_files"][:5]:  # Show first 5
                ...
                print(f"   • {Path(file_path).name} ({result.error_count} errors, {result.warning_count} warnings)")
    ...
    def _analyze_regression(
        ...
        regression_analysis = {
            ...
            "success_rate_change": current_summary.get("success_rate", 0) - baseline_summary.get("success_rate", 0),
            "new_failures": [],
            "resolved_failures": [],
            "status": "unknown",
        }
        ...
        regression_analysis["resolved_failures"] = list(baseline_failed - current_failed)
