...
class CSVSchemaValidator:
    ...
    def _validate_data_content(self, csv_data: List[Dict[str, Any]]) -> List[ValidationIssue]:
    ...
    def _validate_column_data(self, column_name: str, column_data: List[str], field: SchemaField) -> List[ValidationIssue]:
        ...
        for row_idx, value in enumerate(column_data):
            ...
            if field.required and not field.nullable:
                if not value or str(value).strip() == '' or str(value).strip().lower() == 'null':
                    issues.append(ValidationIssue(
                        field_name=column_name,
                        issue_type="required_field_missing",
                        severity="error",
                        description=f"Required field '{column_name}' is missing or null",
                        row_number=row_idx + 2,  # +2 because csv index starts at 0 and has header
    ...
    def _validate_single_value(self, column_name: str, value: str, field: SchemaField, row_number: int) -> List[ValidationIssue]:
...
class BatchValidator:
    ...
    def validate_files(
        self,
        file_schema_pairs: List[Tuple[Union[str, Path], List[SchemaField]]]
    ) -> Dict[str, ValidationResult]:
        """
